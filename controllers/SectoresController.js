...
const get = async (req, res) => {
    ...
    try {
        const data = await Sectores.findOne({
            _id: req.params.id,
        });
...
const add = async (req, res) => {
    ...
    try {
        const data = await Sectores.findOne({ _id: req.body.id });
        if (data) {
            ...
            var registro = new Sectores(updates);
            await registro
                .save()
                .then((data) => res.status(200).json({ data }))
                .catch((error) => res.status(500).json({ message: error }));
...
const update = async (req, res) => {
    ...
    try {
        ...
        const data = await Sectores.findOne({ _id: req.body.id });
        if (data) {
            await Sectores.updateOne({ _id: req.body.id }, { $set: updates })
                .then((data) => res.status(200).json({ data }))
                .catch((error) => res.status(500).json({ message: error }));
        } else {
...
const del = async (req, res) => {
    ...
    try {
        const data = await Sectores.findOne({ _id: req.body.id });
        if (data) {
            await Sectores.deleteOne({ _id: req.body.id })
                .then((data) => res.status(200).json({ data }))
                .catch((error) => res.status(500).json({ message: error }));
        } else {
            res.status(404).send({
