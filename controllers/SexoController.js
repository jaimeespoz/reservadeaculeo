...
const add = async (req, res) => {
    ...
    try {
        const data = await Sexo.findOne({ _id: req.body.id });
        if (data) {
...
const update = async (req, res) => {
    ...
    try {
        ...
        const data = await Sexo.findOne({ _id: req.body.id });
        if (data) {
            await Sexo.updateOne({ _id: req.body.id }, { $set: updates })
                .then((data) => res.status(200).json({ data }))
                .catch((error) => res.status(500).json({ message: error }));
        } else {
...
const del = async (req, res) => {
    ...
    try {
        const data = await Sexo.findOne({ _id: req.body.id });
        if (data) {
            await Sexo.deleteOne({ _id: req.body.id })
                .then((data) => res.status(200).json({ data }))
                .catch((error) => res.status(500).json({ message: error }));
        } else {
            res.status(404).send({
